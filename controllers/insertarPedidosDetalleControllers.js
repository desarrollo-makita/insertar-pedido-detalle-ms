...
async function insertarPedidosDetalle(req, res) {
    ...
    try {
           ...
           const {
            ...
            // Ejecutar el procedimiento almacenado con los parámetros
             result = await request
                ...
                .input('RutCliente', sql.VarChar(20), RutCliente)
        ...
        logger.info(`Fin la funcion insertarPedidosDetalle ${JSON.stringify(responseData)}`);
        res.status(200).json(responseData);
    
    } catch (error) {
       // Manejamos cualquier error ocurrido durante el proceso
       logger.error(`Error en insertarPedidoDetalle: ${error.message}`);
       res.status(500).json({ error: `Error en el servidor [insertar-pedido-detalle-ms] :  ${error.message}`  });
    }
